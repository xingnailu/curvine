...
impl FsWriterBase {
    ...
    fn find_max_block_id(&self) -> FsResult<Option<i64>> {
        ...
        if let Some(file_blocks) = &self.file_blocks {
            for located_block in &file_blocks.block_locs {
                ...
                max_block_id = Some(max_block_id.map_or(block_id, |current_max: i64| current_max.max(block_id)));
            }
        }
    ...
    fn create_commit_block_for_last_block(&self, block_id: i64) -> FsResult<curvine_common::state::CommitBlock> {
        ...
        if let Some(file_blocks) = &self.file_blocks {
            for located_block in &file_blocks.block_locs {
                if located_block.block.id == block_id {
                    ...
                    let locations = located_block.locs.iter()
